...
$Date = Get-Date -Format "MM/dd/yyyy" | ForEach-Object { $_ -replace "/", "." }
...
$Leavers = @()
$Leavers = Get-ADUser -Filter * | Where-Object {($_.enabled -eq $false) -AND ($_.distinguishedname -match "OU=Disabled")} | Select-Object UserPrincipalName
foreach($Leaver in $Leavers){
    ...
    $PW | Add-Member -MemberType ScriptProperty -Name "Password" -Value { ("!@#$%^&*0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ_abcdefghijklmnopqrstuvwxyz".tochararray() | Sort-Object {Get-Random})[0..32] -join '' } -Force
...
#   $ExpiryDate = (Get-date).AddDays(90)
    ...
    $UserExists = Get-ADuser -Identity $UserName
        ...
        Start-Sleep 1
        Write-Host "Removed License Attribute for user:" $UPN
        Set-ADUser -Identity $Username -Clear 'msDS-cloudExtensionAttribute2'
        Start-Sleep 1
        Write-Host "Cleared Manager Attribute for user:" $UPN
        Set-ADUser -Identity $Username -Clear 'manager'
        Start-Sleep 1
        Write-Host "Hid user" $UserDisplayName "- <" $UPN "> from the GAL"
        Set-ADObject -Identity $UserOU -Replace @{msExchHideFromAddressLists=$true}
        Start-Sleep 1
        Write-Host "Updated" $UserDisplayName "DisplayName to:" $NewDisplayName
        Set-ADUser -Identity $UserName -DisplayName $NewDisplayName
        Start-Sleep 1
        Write-Host "Moving User:" $UserDisplayName "from" $UserOU "to Disabled OU"
            ...
            If( $UserOU -match 'OU=Dundas')
            {
            $DundasDOU = "OU=Disabled,OU=Dundas,OU=Users,OU=insightsoftware,DC=insightsoftware,DC=lan"
            Move-ADObject -Identity $UserOU -TargetPath $DundasDOU
            }
            ...
            If( $UserOU -match 'OU=Legerity')
            {
            $LegerityDOU = "OU=Disabled,OU=Legerity,OU=Users,OU=insightsoftware,DC=insightsoftware,DC=lan"
            Move-ADObject -Identity $UserOU -TargetPath $LegerityDOU
            }
