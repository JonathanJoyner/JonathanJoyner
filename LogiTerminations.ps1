...
#####       Updated to work on Logixml.local 4/20/22
...
foreach($Leaver in $Leavers){
    ...
    $FirstName = $Leaver.GivenName
    $Lastname = $Leaver.Surname
    $User = Get-ADUser -Filter * | Where-Object {($_.GivenName -eq $FirstName) -AND ($_.Surname -eq $Lastname)} | Select-Object SamAccountName
